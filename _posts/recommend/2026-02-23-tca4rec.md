...
$$
P(t \mid x_u) = \prod_{j=1}^{|t|} P(y_j \mid x_u, y_{<j})
$$
...
먼저, CF 모델 (SASRec)에서 사용자 임베딩, 아이템 임베딩을 얻고, scoring function으로 아이템에 대한 logit을 생성합니다. 이 과정에서는 Dot product(내적)을 사용해서 아이템별 선호 logit을 구성합니다. 이 과정으로 아이템들 사이에서 사용자 선호도를 충분히 반영할 수 있습니다. 

다음으로, Tokenizer을 만들기 위해, LLM Tokenizer에 텍스트 정보를 반영하여 각 아이템들을 Token sequence로 변환합니다. 

이후 과정에서 Intuitive grouping strategy를 사용해 CF logit을 재구성하고 token-level 확률 분포로 변환합니다. 이 과정에서 LLM이 Autoregressive하게 생성되는데, 현재까지 생성된 prefix와 앞부분이 동일한 아이템만 다음 후보로 인정되도록 수정합니다. 즉, 이전까지 생성된 prefix(토큰)를 보고, 이 정보를 모두 포함하는, 맞게 갖고 있는 아이템만 후보로 남깁니다.
- 그 과정에서 Item logit의 경우, 단순히 더하게 된다면 왜곡 문제가 발생하는데 이 문제를 해결하기 위해 후보 집합 내부에서만 softmax를 적용해 정규화된 아이템 확률 분포로 바꿔줍니다.

이후에, 다음 토큰이 같은 아이템들의 확률을 합쳐 토큰 분포로 만들고, 이 과정으로 인해 토큰이 제대로 정규화되어 조건 식을 만족하게 됩니다. 결과적으로 각 디코딩 step마다 LLM vocab 위의 CF 기반 토큰 확률분포가 생깁니다.
...
- Output : 두 Tokenizer의 분포를 섞어 만든 soft-label
